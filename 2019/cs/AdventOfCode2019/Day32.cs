...
namespace AdventOfCode2019
{
    ...
    class Day32 : Challenge
    {
        ...
        List<Point> DirectionsToPointArray(string directions)
        {
            ...
            Regex pattern = new Regex(@"(?<dir>[UDLR])(?<steps>\d+)");

            string[] directionElements = directions.Split(',');

            foreach (string directionElement in directionElements)
            {
                Match match = pattern.Match(directionElement);
                ...
                switch (match.Groups["dir"].Value)
                {
                    case "U":
                        ...
                        {
                            points.Add(new Point(lastPoint.x, lastPoint.y + i));
                        }
                        break;
                    ...
                    case "L":
                        for (int i = 1; i <= steps; i++)
                        {
                            points.Add(new Point(lastPoint.x - i, lastPoint.y));
                        }
                        break;
