...
programming_language, puzzle_day = parse_cli_options(LANGUAGE_MAPPING.keys, default_language: default_language)
...
puts "We are programming in #{programming_language.colorize(:red)}, AoC #{current_time.year.colorize(:yellow)}, Day #{puzzle_day.colorize(:green)}"

language_tool = LANGUAGE_MAPPING[programming_language].new(day: puzzle_day, year: current_time.year)
